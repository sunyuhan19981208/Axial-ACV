...
class row_attention_block(nn.Module):
    def __init__(self, in_dim, q_k_dim):
        ...
        self.num_heads = 8
        head_dim = self.in_dim // self.num_heads
    ...
    def forward(self, x):
        ...
        hn = self.num_heads
        Q = Q.permute(0,2,3,1,4).contiguous().view(B*D*H, hn, -1, W).permute(0,1,3,2) #size = (b*h*d,hn,w,c2)
        K = K.permute(0,2,3,1,4).contiguous().view(B*D*H, hn, -1, W)  #size = (b*h,c2,w)
        V = V.permute(0,2,3,1,4).contiguous().view(B*D*H, hn, -1, W)  #size = (b*h, c1,w)
        
        #size = (b*h,w,w) [:,i,j] 表示Q的所有h的第 Wi行位置上所有通道值与 K的所有h的第 Wj列位置上的所有通道值的乘积，
        # 即(1,c2) * (c2,1) = (1,1)
        row_attn = (Q @ K) * self.scale # B*D*H, hn, W, W
        ########
        ...
        out = V @ row_attn.permute(0,1,3,2) # B*D*H, hn, 
        #size = (b,c1,h,2)
